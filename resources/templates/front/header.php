<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="https://use.typekit.net/mzo8ook.css">
        <link rel="stylesheet" href="dist/style.css">
        <title>SignBase</title>
    </head>

    <body></body>
    <header class="header">
        <div class="top-area flex flex-jc-sb flex-ai-c">
            <div class="search-bar">search bar</div>
            <a href="login.php"><img class="login" src="assets/logo/sb_baba.png"></a>
        </div>
        <nav>
            <ul class="header__links flex flex-jc-sa flex-ai-c">
                <li><a href="#">ABOUT</a></li>
                <li><a href="products.php">PRODUCTS</a></li> 
                <li><a href="index.php"><img class="header__links--logo" src="assets/logo/logo_white.png" alt="SIGNBASE"></a></li>
                <li><a href="#">FAQ</a></li>
                <li><a href="#">CONTACT</a></li>
            </ul>
        </nav>
        <div class="header__menu"></div>
    </header>